...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const logged_user_details = await fetch(MONGODB_LOGGED_USER);
      const user_data = await logged_user_details.json();
      setLoggedInUser(user_data.decodedToken);
    };
    fetchData();
  }, []);
  ...
  const logout = async () => {
    try {
      const user_logout = await fetch(MONGODB_LOGOUT);
    ...
    <nav className="bg-white border-white dark:bg-white fixed top-0 w-full">
      <div className="flex flex-wrap items-center justify-between mx-auto p-4 px-16">
          ...
          <div>
            ...
            <div className='hidden w-full md:block md:w-auto mt-1'>
              <ul className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-white md:dark:bg-white dark:border-gray-700">
                {currentPath === LOGIN_URL || currentPath === FORGOT_PASSWORD || currentPath === SIGNUP_URL ? '' :
                  <li className={`profile_dropdown relative ${isDropdownOpen === true ? 'border-b-4 border-b-red-500 h-12 -mb-4 w-10' : ''}`} onMouseEnter={() => setIsDropdownOpen(true)} onMouseLeave={() => setIsDropdownOpen(false)}><button className="py-2 px-3 text-black rounded hover:bg-white md:hover:text-blue-700 md:p-0 md:dark:hover:text-blue-500 dark:text-black dark:hover:bg-white hover:cursor-pointer dark:hover:text-white grid gap-1 justify-items-center" ><FontAwesomeIcon icon={faUser} /><span className='text-xs'>Profile</span></button>
                    {isDropdownOpen && (
                      <div className="absolute w-72 -right-28 top-12 border-2 border-solid border-gray-100 p-3 bg-gray-50 rounded shadow-md">
                        <div className='p-3'>
                          {loggedInUser && Object.keys(loggedInUser).length > 0 ? (
                            <div>
                              <p className='font-bold py-1'>Welcome {loggedInUser.name}</p>
                              <p className='py-1 my-2 border-2 border-gray-200 text-center text-pink-500 uppercase font-bold'>
                                <Link href="#" className='redirect_login_page text-sm cursor-pointer' onClick={() => logout()}>Logout</Link>
                              </p>
                            </div>
                          ) : (
                            <div>
                              ...
                              <p className='py-1 my-2 border-2 border-gray-200 text-center text-pink-500 uppercase font-bold'>
                                <Link href={LOGIN_URL} className='redirect_login_page text-sm'>Login/ Signup</Link>
                              </p>
                            </div>
                          )}
                        </div>
        ...
        <div className="items-center justify-between hidden md:flex md:order-1 lg:w-1/3 w-3/5" id="categories_data">
          {categoryHeader?.map((category_header) => (
            category_header.status === 1 && (
              <ul key={category_header._id} className="flex flex-col p-4 md:p-0 mt-4 font-medium border border-gray-100 rounded-lg bg-gray-50 md:space-x-8 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0 md:bg-white dark:bg-white md:dark:bg-white dark:border-gray-700">
