...
const AddCategorytypes = () => {
    ...
    useEffect(() => {
        const fetchCategoryHeaderData = async () => {
            const responseData = await fetch(MONGODB_CATEGORY_HEADING);
            const response = await responseData.json();
            setCategoryHeaders(response.category_heading)
        };
    ...
    const addCategoryTypesForm = async (e) => {
        e.preventDefault();
        const validation_errors = validate_category_types(formData);
        if (Object.keys(validation_errors).length === 0) {
            try {
                const response = await axios.post(MONGODB_CATEGORY_TYPES, formData);
    ...
    return (
        <div className='add_category_types_page sm:ml-60'>
            <div className="add_category_types_title">
                <h1 className='text-5xl font-bold leading-loose text-center py-3'>Add Category Types</h1>
            </div>
                    ...
                    <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                        ...
                        <li aria-current="page">
                            <div className="flex items-center">
                                ...
                                <span className="ms-1 text-sm font-medium text-black md:ms-2 dark:text-gray-400">Add Category Type</span>
                            </div>
                        </li>
                    </ol>
                </div>
            ...
            <div className="add_category_types_form w-2/5 sm:ml-96">
                <form method='POST' onSubmit={addCategoryTypesForm}>
                            ...
                            <fieldset>
                                ...
                                <div className="relative border border-gray-300 text-gray-800 shadow-lg rounded-sm">
                                    ...
                                    <select className="category_heading_id appearance-none w-full py-1 px-2 rounded-sm" name="category_heading_id" id="category_heading_id" value={formData.category_heading_id} onChange={handleInputChange} >
                                        ...
                                        {categoryHeaders.map((category_header) => (
                                            <option key={category_header._id} value={category_header._id}>{category_header.name}</option>
                                        ))}
                                    </select>
                    ...
                    <div className='submit_button'>
                        <SubmitButton className="add_category_types_button" id="add_category_types_button" name="add_category_types_button" div_name="add_category_types_button" label="Submit" />
                    </div>
                </form>
            </div>
        </div>
